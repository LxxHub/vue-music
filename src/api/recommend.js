...
export function getRecommend () {
  ...
  const url = debug ? '/api/getTopBanner' : 'http://123.207.196.15:8089/music/api/getTopBanner'
  ...
  return axios.get(url, { params: data }).then((res) => { return res.data })
}

export function getDiscList () {
  // 线上环境地址，同学们根据自己的需要配置修改
  const url = debug ? '/api/getDiscList' : 'http://123.207.196.15:8089/music/api/getDiscList'
  const data = Object.assign({}, commonParams, {
    platform: 'yqq',
    hostUin: 0,
    sin: 0,
...
export function getSongList (disstid) {
  const url = debug ? '/api/getCdInfo' : 'http://123.207.196.15:8089/music/api/getCdInfo'
